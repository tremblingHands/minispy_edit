...
#pragma prefast(suppress:__WARNING_BANNED_API_USAGE, "reviewed and safe usage")
		...
		if (Data->Iopb->MajorFunction == IRP_MJ_WRITE) {
			/*
			recordData->Arg5 = Data->Iopb->Parameters.Write.MdlAddress;
			if (Data->Iopb->Parameters.Write.MdlAddress != NULL) {
				recordData->Arg5 = (PUCHAR)MmGetSystemAddressForMdlSafe(Data->Iopb->Parameters.Write.MdlAddress, NormalPagePriority);
			}
			*/
			...
			if (Data->Iopb->Parameters.Write.MdlAddress != NULL) {
				RtlInitAnsiString(&buf ,(PCHAR )MmGetSystemAddressForMdlSafe(Data->Iopb->Parameters.Write.MdlAddress, NormalPagePriority));
			}
			else {
				RtlInitAnsiString(&buf, (PCHAR )Data->Iopb->Parameters.Write.WriteBuffer);
			}



			UNICODE_STRING nFilePath;
			UNICODE_STRING EnterString;
			RtlInitUnicodeString(&EnterString, L"\r\n");
			//RtlAppendUnicodeStringToString(nameToUse, &EnterString);

			RtlInitUnicodeString(&nFilePath, L"\\??\\c:\\write_content.txt");//初始化文件名    
			InitializeObjectAttributes(&obj_attrib, &nFilePath, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE,
				NULL, NULL);

			//create file  
			lstatus = ZwCreateFile(&lhFile, FILE_APPEND_DATA, &obj_attrib, &Io_Status_Block, NULL,
				FILE_ATTRIBUTE_NORMAL, FILE_SHARE_READ,
				FILE_OPEN_IF, FILE_NON_DIRECTORY_FILE | FILE_SYNCHRONOUS_IO_NONALERT,
				NULL, 0);
			/*
			UNICODE_STRING UnicodeString2;
			UnicodeString2.Buffer = (PWSTR)ExAllocatePool(PagedPool, 10);
			UnicodeString2.MaximumLength = 10;
			RtlIntegerToUnicodeString(buf.Length,10,&UnicodeString2);
			*/
			if (!NT_SUCCESS(lstatus))
			{
			...
			//RtlFreeAnsiString(&buf);
			//RtlFreeUnicodeString(&nFilePath);
		}
